...
import { AuthAction, AuthActionType, authReducer, AuthState, initialState } from '~/reducers/auth'
...
export function AuthProvider(props: React.PropsWithChildren<any>): JSX.Element {
  ...
  function signIn(username: string, password: string): Promise<IUser> {
    return authAxios.post<IUser>('signin', { username, password }).then((response) => response.data)
  }
  ...
  return <CtxProvider value={{ state, dispatch, getUser, signIn, signUp, signOut }} {...props} />
}
